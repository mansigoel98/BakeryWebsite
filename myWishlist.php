...
.btremove:hover {
  ...
  color:white;
       ...
       <body>
        ...
        <!------------------------------------next section------------------------------------------------------------>
       <h1 class="annh">Your WishList</h1>
       <div class="cartdiv"> 
        <?php
        $username=$_SESSION['username'];
        $con =mysqli_connect('localhost','mansi','mansi');
       if($con)
           {
              mysqli_select_db($con,'bakery');
              $q = "select * from wishlist where username='$username'";
              $result = mysqli_query($con,$q);
              $num=mysqli_num_rows($result);
              if($num==0){
                echo "<h1 class='annh'>Your Wishlist is empty</h1>";
              }
              else{
               ...
               $price=$row['price'];
                ...
                <button type="submit" class="btremove"  name="removewish" value ="<?php echo "$cakeid"  ?>" formaction="removeWish.php">Remove</button>
               </form>
                </div>
